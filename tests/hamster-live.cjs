...
(async()=>{try{
 ...
 if(process.env.HAMSTER_BROWSER_URL){
  ...
  const browser=await chromium.launch({headless:true});try{
   ...
   require('node:fs').mkdirSync('test-results/hamster',{recursive:true});await page.waitForFunction(()=>{const i=document.querySelector('.reel-mascot .mascot');return i.complete&&i.naturalWidth>0;});await page.screenshot({path:'test-results/hamster/published-mobile.png'});await page.locator('.bottom-nav [data-tab="hamster"]').click();await page.locator('[data-action="whatsapp-preferences"]').click();assert.equal(await page.locator('#whatsapp-number').inputValue(),'+996700123456');assert.equal(await page.locator('#whatsapp-optin').isChecked(),false);await page.locator('#whatsapp-confirm').check();await page.locator('#whatsapp-optin').check();await page.locator('[data-action="save-whatsapp"]').click();await page.waitForSelector('#whatsapp-form',{state:'detached'});await call(staff,'whatsappDraft',{uid:customer.localId},true);await page.locator('[data-action="whatsapp-preferences"]').click();assert.equal(await page.locator('#whatsapp-optin').isChecked(),true);await page.locator('#whatsapp-confirm').check();await page.locator('#whatsapp-optin').uncheck();await page.locator('[data-action="save-whatsapp"]').click();await page.waitForSelector('#whatsapp-form',{state:'detached'});await assert.rejects(call(staff,'whatsappDraft',{uid:customer.localId},true));console.log('PUBLIC BROWSER PASS: saved game, stable mobile layout, WhatsApp opt-in and opt-out persisted with server enforcement.');
