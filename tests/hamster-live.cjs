...
(async()=>{try{
 ...
 if(process.env.HAMSTER_BROWSER_URL){
  ...
  const browser=await chromium.launch({headless:true});try{
   ...
   require('node:fs').mkdirSync('test-results/hamster',{recursive:true});await page.waitForFunction(()=>{const i=document.querySelector('.reel-mascot .mascot');return i.complete&&i.naturalWidth>0;});await page.screenshot({path:'test-results/hamster/published-mobile.png'});console.log('PUBLIC BROWSER PASS: signed-in Firestore game, visible spin button, stable mobile widths, reload persistence.');
